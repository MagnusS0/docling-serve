...
class ChunkedDocumentResponseItem(BaseModel):
    report_chunk_id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    report_id: str
    file_name: str
    title: Optional[List[str]] = None # List of headings for the chunk
    chunk: str
...
def process_chunked_results(
    background_tasks: BackgroundTasks,
    conversion_options: ConvertDocumentsOptions,
    conv_results: Iterable[ConversionResult],
    report_id: str, # Add report_id to link chunks to a report
) -> ChunkedDocumentResponse:
    """Processes document conversion results and prepares chunked output for vector DB."""

    try:
        start_time = time.monotonic()
        conv_results_list = list(conv_results) # Convert iterator to list to ensure processing
        processing_time = time.monotonic() - start_time
        _log.info(f"Processed {len(conv_results_list)} docs for chunking in {processing_time:.2f} seconds.")

    except Exception as e:
        _log.error(f"Error during document processing for chunking: {e}")
        raise HTTPException(status_code=500, detail=str(e))

    if not conv_results_list:
        raise HTTPException(
            status_code=500, detail="No documents were generated by Docling for chunking."
        )

    all_chunks = []
    total_errors = []
    overall_status = ConversionStatus.SUCCESS # Assume success, downgrade if errors

    chunker = HybridChunker(tokenizer=conversion_options.chunking_tokenizer)
    ...
    for conv_res in conv_results_list:
        if conv_res.status == ConversionStatus.SUCCESS:
            ...
            for chunk in chunks:
                ...
                chunk_titles = chunk.meta.headings if hasattr(chunk.meta, 'headings') else None # Safely access headings
                chunk_item = ChunkedDocumentResponseItem(
                    report_id=report_id,
                    file_name=conv_res.input.file.name,
                    title=chunk_titles,
                    chunk=chunk_text,
                    report_chunk_id=str(uuid.uuid4()) # Generate unique chunk ID
                )
                all_chunks.append(chunk_item)
        else:
            overall_status = ConversionStatus.PARTIAL_SUCCESS if overall_status == ConversionStatus.SUCCESS else overall_status # Downgrade status
            total_errors.extend(conv_res.errors) # Collect errors
            _log.warning(f"Document {conv_res.input.file} failed conversion, chunks may be incomplete.")
    ...
    return ChunkedDocumentResponse(
        ...
        errors=total_errors,
        processing_time=processing_time,
        timings=conv_results_list[0].timings if conv_results_list and conv_results_list[0].timings else {}, # Take timings from first result if available
    )